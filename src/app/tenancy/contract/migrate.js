...
async function migrateData() {
  try {
    ...
    const propertiesData = JSON.parse(await fs.readFile('src/app/property/properties/list/properties-data.json', 'utf8'));
    const tenantsData = JSON.parse(await fs.readFile('src/app/tenancy/tenants/tenants-data.json', 'utf8'));
    ...
    for (const tenant of tenantsData) {
      await tenantsCollection.doc(tenant.id).set(tenant);
    }
