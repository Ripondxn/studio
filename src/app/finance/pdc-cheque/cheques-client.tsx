...
export function PdcChequesClient({ initialCheques, initialSummary }: { initialCheques: PdcCheque[], initialSummary: Summary }) {
  ...
  const handleExportPDF = () => {
    ...
    const body = cheques.map(cheque => 
        exportableColumns.map(col => {
            // @ts-ignore
            const value = cheque[col.accessorKey as keyof PdcCheque];
    ...
    doc.text("PDC Cheques List", 14, 16);
    doc.autoTable({
        head: head,
        body: body,
        startY: 20,
    });
    doc.save('pdc-cheques.pdf');
  };
  ...
  const handleExportExcel = () => {
    const dataToExport = cheques.map(cheque => {
        let obj: any = {};
         columns.filter(col => 'accessorKey' in col && col.accessorKey && col.id !== 'actions').forEach(col => {
            ...
            // @ts-ignore
            obj[headerText] = cheque[col.accessorKey as keyof PdcCheque];
         })
         return obj;
    });
    ...
    XLSX.utils.book_append_sheet(wb, ws, "PDC Cheques");
    XLSX.writeFile(wb, "pdc-cheques.xlsx");
  };
  ...
  return (
    <div className="container mx-auto py-10">
      <div className="flex justify-between items-center mb-6">
        <div>
            <h1 className="text-3xl font-bold font-headline">PDC Cheque Management</h1>
