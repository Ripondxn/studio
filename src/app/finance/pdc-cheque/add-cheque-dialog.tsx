...
import { addPdcCheque, getLookups } from './actions';
...
type ChequeFormData = Omit<PdcCheque, 'id'>;
const chequeFormSchema = pdcChequeSchema.omit({ id: true });
...
export function AddChequeDialog({ onChequeAdded }: { onChequeAdded: () => void }) {
  ...
  const onSubmit = async (data: ChequeFormData) => {
    setIsSaving(true);
    const result = await addPdcCheque(data);
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-xl">
        <form onSubmit={handleSubmit(onSubmit)}>
            <DialogHeader>
            <DialogTitle>Add New PDC Cheque</DialogTitle>
            <DialogDescription>
                Fill in the details below to record a new post-dated cheque. Select a contract to auto-fill details.
            </DialogDescription>
            </DialogHeader>
