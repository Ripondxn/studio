...
import { importRooms } from './actions';
...
const requiredHeaders = ['roomCode'];

export function ImportRoomsDialog({ propertyCode, onImportSuccess }: ImportRoomsDialogProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [parsedData, setParsedData] = useState<Omit<Room, 'id' | 'occupancyStatus' | 'propertyCode'>[]>([]);
  ...
  const handleImport = async () => {
    ...
    const roomsToImport = parsedData.map(room => ({
        ...room,
        propertyCode
    }));
    
    const result = await importRooms(roomsToImport);

    if (result.success) {
      toast({
        title: 'Import Successful',
        description: `${result.added} rooms added, ${result.updated} rooms updated.`,
      });
      onImportSuccess();
      setIsOpen(false);
      setParsedData([]);
  ...
  const displayHeaders = ['roomCode', 'roomName', 'floorCode', 'unitCode', 'roomType', 'rentAmount'];
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Select an Excel file to bulk-add or update rooms for property {propertyCode}.
